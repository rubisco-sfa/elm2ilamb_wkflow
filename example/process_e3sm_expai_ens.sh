...
for ni in `seq 1 36`; do
    ...
    for cname in "${CaseNames[@]}"; do
        ...
        # --------------------------------------------------------------------------------
        # in E3SM, the NBP did not include all disturbances, instead of NEE. So NBP = -NEE

        cdir=${CmrDir}/${cname}/org/lnd_${cinst}/
        if [ -f  ${cdir}/NEE_199101_201412.nc ]; then
             /bin/rm -f ${cdir}/*.tmp
             /bin/rm -f ${cdir}/tmp.nc
             /bin/cp -f ${cdir}/NEE_199101_201412.nc ${cdir}/tmp.nc
             ncap2 -O -s 'NEE=-NEE' ${cdir}/tmp.nc -o ${cdir}/NBP_199101_201412.nc
             ncrename -v NEE,NBP ${cdir}/NBP_199101_201412.nc
             /bin/rm -f ${cdir}/tmp.nc
        fi
    done
