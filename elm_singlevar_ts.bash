...
print_usage () {
   ...
   echo -e "         \e[1m--year_range, -y       \e[0m:the year range of the simulations used in the name construction of the output files.
   ...
   echo -e "         \e[1m--no-gen-ts            \e[0m: switch of not generating ts (i.e. they were generated before)"
   echo -e "         \e[1m--skip_genmap          \e[0m: 0 mean not skip map generation, positive integer number is to skip and use mapXXXX.nc"
   ...
   echo -e "         \e[1m--caseidpath, -i       \e[0m: the directory of the simulations provided atm and lnd directories seperated by comma if they are different"
   echo -e "         \e[1m--outputpath -o        \e[0m: the directory for the outputs generated by this script"
   echo -e "         \e[1m--experiment, -e       \e[0m: the experiment name following CMIP conventions used in the name constructions of the output files"
   echo -e "         \e[1m--model, -m            \e[0m: the model name with special features, for example ACME_WCYCL used as the subdirectory name under outputpath to save output files"
   ...
   echo -e "         \e[1m--ncreamp|creamp3      \e[0m: switch of remapping methods either using ncremap or conv_remap3, cannot be used with linkfil together"
   echo -e "         \e[1m--linkfil              \e[0m: switch to link the original files to remapped file instead of doing actual remapping"
   echo -e "         \e[1m--cmip                 \e[0m: switch to rewrite model outputs following CMIP conventions"
   echo -e "         \e[1m--skip_rename          \e[0m: switch to skip renaming for cmip conversion"
...
drc_cmor=${drc_rgr}/CMOR   # CMOR directory
